...
public class PointingScript_B_Desktop : MonoBehaviour
{
    ...
    private string fileName = "pointing_desktop_b_" + experimentFlow.participant + DateTime.Now.ToString("_yyyy-MM-dd_hhmm") + ".txt";
    private string path;
    ...
    void Update()
    {
        if (Input.GetButtonDown("ps4x"))
        {
            ...
            if (Input.GetButtonDown("ps4x"))
            {
                // Replace "ps4x" with the appropriate button name for your joystick input
        ...
        else if (targetBuildingIndicesRemaining.Count == 0)
        {
            // Rotate the camera to face north when there are no more buildings to show
